...
// Use Unity physics engine to simulate forces while grabbing.
// As opposed to KinematicGrabber, object retains its physics properties also when grabbed.
// Downside: grabbing is not stable as objects slip out of hand if too much force is applied.
// To slightly decrease object movement stuttering, set Interpolation in rigidbody.
/*
public class PhysicsGrabber : BaseGrabber
{
    ...
    void CheckGrabOrRelease()
    {
        ...
        bool wasGrabbing = (State == HandState.GRABBING);
    ...
    private void MovegrabbedObjectWithForce()
    {
        ...
        rb.linearVelocity = (handReference.transform.position - rb.transform.position) / Time.deltaTime;
    ...
    {
        foreach (HandJointId thumbBone in ThumbCollisionStartJoints)
        {
            CurrentCollisions.TryGetValue(thumbBone, out Collider collisionWithThumb);
            if (collisionWithThumb)
    ...
    protected bool IsOppositeGrabbing(Collider grabbableCollider)
    {
        ...
        {
            foreach (HandJointId oppositeBone in OppositeCollisionStartJoints)
            {
